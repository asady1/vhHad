...
#number total events
parser.add_option("-n", "--ntevt", dest="ntevt", 
		  help="number total events")
...
mynewTree.Branch('nTotEvents', nTotEvents, 'nTotEvents/F')

nevent = treeMine.GetEntries();

hJetVMass = ROOT.TH1F("hJetVMass", "V Jet Pruned Mass (GeV)", 20, 60, 110)
hJetHMass = ROOT.TH1F("hJetHMass", "H Jet Pruned Mass (GeV)", 30, 0, 400)
hJetVbbtag = ROOT.TH1F("hJetVbbtag", "V Jet Double b Discriminant (GeV)", 20, -1, 1)
hJetHbbtag = ROOT.TH1F("hJetHbbtag", "H Jet Double b Discriminant (GeV)", 20, -1, 1)
hJetVtau21 = ROOT.TH1F("hJetVtau21", "V Jet tau21", 20, 0, 1)
hJetHtau21 = ROOT.TH1F("hJetHtau21", "H Jet tau21", 20, 0, 1)
hJetVtau21DDT = ROOT.TH1F("hJetVtau21DDT", "V Jet tau21 DDT", 20, 0, 1)
hJetHtau21DDT = ROOT.TH1F("hJetHtau21DDT", "H Jet tau21 DDT", 20, 0, 1)
...
for i in range(0, nevent) :
    ...
    if jetV == 1: # jet1 is v jet, so jet 2 must be h jet
        hJetVMass.Fill(treeMine.jet1pmass)
        hJetHMass.Fill(treeMine.jet2pmass)
        hJetVbbtag.Fill(treeMine.jet1bbtag)
        hJetHbbtag.Fill(treeMine.jet2bbtag)
        hJetVtau21.Fill(treeMine.jet1tau21)
        hJetHtau21.Fill(treeMine.jet2tau21)
        if treeMine.jet1pmass > 0 and treeMine.jet1pt > 0:
            hJetVtau21DDT.Fill(treeMine.jet1tau21 + 0.063*math.log((treeMine.jet1pmass**2)/treeMine.jet1pt))
        if treeMine.jet2pmass > 0 and treeMine.jet2pt > 0:
            hJetHtau21DDT.Fill(treeMine.jet2tau21 + 0.063*math.log((treeMine.jet2pmass**2)/treeMine.jet2pt)) 
    ...
    if jetV == 2: # jet2 is v jet, so jet 1 must be h jet
        hJetHMass.Fill(treeMine.jet1pmass)
        hJetVMass.Fill(treeMine.jet2pmass)
        hJetHbbtag.Fill(treeMine.jet1bbtag)
        hJetVbbtag.Fill(treeMine.jet2bbtag)
        hJetHtau21.Fill(treeMine.jet1tau21)
        hJetVtau21.Fill(treeMine.jet2tau21)
        if treeMine.jet1pmass >0 and treeMine.jet1pt >0:
            hJetHtau21DDT.Fill(treeMine.jet1tau21 + 0.063*math.log((treeMine.jet1pmass**2)/treeMine.jet1pt))
        if treeMine.jet2pmass >0 and treeMine.jet2pt >0:
            hJetVtau21DDT.Fill(treeMine.jet2tau21 + 0.063*math.log((treeMine.jet2pmass**2)/treeMine.jet2pt)) 
    ...
    nTotEvents[0] = float(options.ntevt)
